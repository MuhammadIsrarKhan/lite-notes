          ...
          <span class="px-2 py-1 border border-indigo-400 bg-indigo-100 rounded font-semibold text-sm">{{ $note->notebook->name }}</span>
            <div class="flex gap-6">
              ...
              <form action="{{ route('notes.destroy',$note) }}" method="post">
                @method('delete')
                @csrf
                <x-primary-button onclick="return confirm('Are you sure you want to delete this note?')" class="!bg-red-500 hover:bg-red-600 focus:bg-red-600" >Delete Note</x-primary-button>
              </form>
            </div>
