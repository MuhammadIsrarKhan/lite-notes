            ...
            <div class="bg-white p-6 overflow-hidden shadow-sm sm:rounded-lg max-w-2xl">
                <form action="{{ route('notes.update', $note) }}" method="post">
                    ...
                    <x-textarea name="text" placeholder="Type your note" rows="8"
                        class="w-full mt-6">{{ old('text', $note->text) }}</x-textarea>
                    @error('text')
                        <div class="text-sm mt-1 text-red-500">{{ $message }}</div>
                    @enderror
                    <select name="notebook_id" id=""
                        class="w-full mt-6 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        <option value="">Select Notebook</option>
                        @foreach ($notebooks as $notebook)
                            <option value="{{ $notebook->id }}" @if ($notebook->id === $note->note_book_id)
                                selected
                            @endif>{{ $notebook->name }}</option>
                        @endforeach
                    </select>
